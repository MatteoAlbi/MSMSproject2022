classdef DRS2 < DAC_ODEclass
  ...
  methods
      function self = DRS2(Iz_flap, L0, L2, L3, L4, LMP, Lbase, g, m_flap, xE, yE, m_piston, ForcePiston)
      neq  = 5;
      ninv = 3;
    ...
    function res__DfDx = DfDx( self, t, vars__ )
      ...
      t154 = L4 * lambda1;
      res__4_1 = 0.1e1 / t112 * (8 * t10 * t4 * t154 + 8 * t28 * t5 * t154 + 4 * Torque_wing_interpolated(theta3));
    ...
    function res__h = h( self, t, vars__ )
      ...
      t123 = t1 * ForcePiston;
      t129 = t1 ^ 2;
      res__3 = 0.1e1 / m_piston / (t41 + 4 * Iz_flap) * (16 * t48 * (-t41 / 4 + (s + L0 + L2 - xE - yE + LMP + Lbase) * (s + L0 + L2 + xE - yE - LMP + Lbase) * m_piston - Iz_flap) * t11 * lambda1 + 32 * t8 * (t3 * t1 * t6 * lambda1 * L4 + t11 * t59 * m_flap * omega3 / 16 + Iz_flap * t59 * omega3 / 4 + t1 * Torque_wing_interpolated(theta3) / 4) * m_piston * L4 - 2 * t3 * (t11 * (4 * lambda1 * t1 + m_piston * t75 - ForcePiston) * m_flap + 16 * lambda1 * t1 * Iz_flap + m_piston * (4 * Iz_flap * t75 - 4 * t6 * Torque_wing_interpolated(theta3)) - 4 * Iz_flap * ForcePiston) * L4 + 4 * m_flap * lambda1 * t94 + t11 * (lambda1 * (16 * m_piston * t98 + 16 * Iz_flap + m_flap * (-8 * L0 * t17 - 8 * L2 * t21 - 8 * Lbase * t13 + 4 * t12 + 4 * t16 + 4 * t20 + 4 * t24 - 8 * t25 + 4 * t27)) - 2 * m_flap * (t122 + t123)) + 16 * (lambda1 * t129 - t122 / 2 - t123 / 2) * Iz_flap);
    ...
    function res__DhDx = DhDx( self, t, vars__ )
      ...
      t78 = 0.1e1 / t52;
      t79 = 0.1e1 / m_piston;
      res__3_1 = -32 * t79 * t78 * L4 * (-2 * t24 * t23 * m_piston * L4 * lambda1 + t2 * (t5 * t36 * t28 + m_piston * (t29 * t1 * t39 * m_flap / 16 + Iz_flap * t1 * t39 / 4 - t4 * Torque_wing_interpolated(theta3) / 4) + (t53 - t54) * t52 / 4) + (t5 * (t29 * t62 * m_flap * omega3 / 16 + Iz_flap * t62 * omega3 / 4 + t1 * Torque_wing_interpolated(theta3) / 4) + t23 * t28) * m_piston);
      t98 = t31 + Iz_flap;
      res__3_2 = t79 * t78 * (32 * t24 * t29 * m_piston * t53 + 32 * t18 * m_piston * (t6 * t28 + Torque_wing_interpolated(theta3) / 4) + 32 * t98 * (-t5 * (m_piston * t39 + 4 * lambda1) * L4 / 4 + t53 - t54));
      t114 = t29 ^ 2;
      t117 = t4 ^ 2;
      t121 = Lbase ^ 2;
      t126 = L0 ^ 2;
